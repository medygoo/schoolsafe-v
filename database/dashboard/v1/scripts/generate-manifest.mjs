...
const manifest = { dashboard_version: "schoolsafe-dashboard-v1", requires_baseline: "schoolsafe-vps-v1", units };
await writeFile(path.join(dir, "manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
await writeFile(path.join(dir, "manifest.sha256"), `${units.map(u => `${u.sha256}  ${u.file}`).join("\n")}\n`, "utf8");
console.log(`Wrote deterministic checksums for ${units.length} dashboard units.`);