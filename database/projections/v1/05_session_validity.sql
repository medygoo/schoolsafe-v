...
-- A5.0: preserve the existing bootstrap contract, but describe only currently
-- active assignments. This is navigation metadata, never an action authorization.
-- A5.1 must represent targeted/conditional DENY without flattening it globally.
...
begin
  ...
  with current_roles as materialized (
    ...
    select g.id,g.effect,p.code from iam.role_permission_grants g
    ...
    select ex.id,ex.effect,ex.reason,ex.expires_at,p.code from iam.profile_permission_exceptions ex
